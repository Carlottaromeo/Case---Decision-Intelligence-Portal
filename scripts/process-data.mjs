...
} from "./process-logic.mjs"
import { normalizeDepartmentName, normalizeSeniority } from "./dept-normalize.mjs"
...
for (const e of employees) {
  ...
  EMPLOYEE_ROSTER.push({ employee_id: id, department: dept, seniority })
...
for (let i = 1; i < lines.length; i++) {
  ...
  if (!seniorityAgg[sen]) {
    seniorityAgg[sen] = { users: new Set(), credits: 0 }
  }
...
const WEEKLY = [...weeklyMap.entries()]
  ...
  .map(([, v]) => ({ week: fmtWeek(v.date), credits: v.credits, sessions: v.sessions, active_users: v.activeUsers.size }))
...
const PROCESS_MAPS = PROCESS_DEFINITIONS.map(({ department, process_name }) => {
  ...
  const avg_sessions_per_active_user = active_users > 0
    ? Math.round(d.total_sessions / active_users)
    : 0
  const avg_credits_per_active_user = active_users > 0
    ? Math.round(d.total_credits / active_users)
    : 0
...
const output = `// ─── DATI GENERATI AUTOMATICAMENTE ─────────────────────────────────────────
...
export const WEEKLY = ${JSON.stringify(WEEKLY, null, 2)};

export const DEPT = ${JSON.stringify(DEPT, null, 2)};

export const TOOL_DATA = ${JSON.stringify(TOOL_DATA, null, 2)};

export const LLM_DATA = ${JSON.stringify(LLM_DATA, null, 2)};

export const USER_SEGMENTS = ${JSON.stringify(USER_SEGMENTS, null, 2)};

export const SENIORITY = ${JSON.stringify(SENIORITY, null, 2)};

export const KPIs = ${JSON.stringify(KPIs, null, 2)};

export const DATA_QUALITY = ${JSON.stringify(DATA_QUALITY, null, 2)};

export const PROCESS_MAPS = ${JSON.stringify(PROCESS_MAPS, null, 2)};

export const EMPLOYEE_ROSTER = ${JSON.stringify(EMPLOYEE_ROSTER, null, 2)};

export const USAGE_RECORDS = ${JSON.stringify(USAGE_RECORDS, null, 2)};

export const PROCESS_WEEKS = ${JSON.stringify(PROCESS_WEEKS, null, 2)};

export const DEPT_COLORS = ${JSON.stringify(DEPT_COLORS, null, 2)};
`

fs.writeFileSync(OUT_PATH, output)
console.log("✓ data.js generato")
console.log("  Dipendenti:", totalEmployees)
console.log("  Provisionati:", provisioned)
console.log("  Crediti totali:", totalCredits.toLocaleString())
console.log("  Sessioni:", totalSessions.toLocaleString())
console.log("  Settimane:", weeks)
console.log("  Crescita:", growthPct + "%")
console.log("  Unmapped provisioned:", DATA_QUALITY.unmapped_provisioned)
